...
VOID StartTimer(Tools::TimerInfo& timer, int timeOffset)
{
	...
	int interval = HOUR_IN_MILLISECONDS; //interval v hodin·ch , interval in hours
...
VOID CheckAndModifyTimer(Tools::TimerInfo& timer)
{
    ...
    liDueTime.QuadPart = -static_cast<LONGLONG>(timer.interval) * HOUR_IN_100NS; // p¯evod hodin na 100-nanosekundovÈ intervaly, convert hours to 100-nanosecond intervals
    int interval = HOUR_IN_MILLISECONDS; //interval v hodin·ch, interval in hours
